...
app.post('/check-high-priority', async (req, res) => {
    ...
    if (!highPriorityKeywords || !previewText) {
        return res.status(400).json({ error: 'Missing highPriorityKeywords or previewText' });
    }

    const prompt = `These are high priority keywords ${JSON.stringify(highPriorityKeywords)}.
...
Kindly tell if the preview text is of high priority. Answer "yes" or "no" and provide an array of most suitable one or at maximum two keywords matched.`;
    ...
    try {
        ...
        if (match) {
            ...
            return res.status(500).json({ error: "Unexpected response format", raw: responseText });
        }
